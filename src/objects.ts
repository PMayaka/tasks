...
export function isCorrect(question: Question, answer: string): boolean {
    let condition = false;
    const stringLower = question.expected.trim();
    const stringTrim = stringLower.toLowerCase();
    if (stringTrim === answer.trim().toLowerCase()) {
        condition = true;
    }
    return condition;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "short_answer_question") {
        ...
        condition = question.options.includes(answer);
    }
    return condition;
}
...
export function toShortForm(question: Question): string {
    const x = question.name.trim();
    const shortName = x.substring(0, 10);
    const string = question.id.toString() + ": " + shortName;
    return string;
}
